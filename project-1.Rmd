...
ggplot(
  ...
  theme_minimal()
```
This bar chart shows that while most people don't do hard drugs, more people in low poverty (more poverty) have recorded hard drug use.
...
This graph reveals that the respondants who smoke have a similar sleeping schedule to those who smoke, seen by the median who both average moe than 7.0 hours. Apart from the density, the graph interestingly reveals that there is really not a significant difference in sleep reported by smokers and non-smokers. 
...
g <- ggplot(subset(nhanes_clean, HealthGen %in% answered), aes(x = HealthGen, y = AlcoholYear, fill = HealthGen))
    g + geom_boxplot(alpha = 0.7) + coord_flip() +  scale_fill_brewer(palette = "Spectral", direction = -1) +
    labs(title = "Distribution of self-related health by alcohol consumption level", x = "self-reported health", y = "Average
    Alcohol Use (drinks per year)") + theme(legend.position = "none")
```

The boxplots reveal that people who reported to consume more alcohol per year reported themselves to be in "excellent" condition, this is noted by the median. Wherear people who consume less alcoholic beverages per year tend to report themselves to be in "poor" or "fair" conditions
...
*6.  Faceted scatter plot of sleep duration vs. age, faceted by gender and marital status*

To explore relationships between variables, we examined how socioeconomic status and lifestyle factors interact with one another and with health outcomes.

Heatmap of average daily alcohol use by age group and marital status *
...
This heatmap shows how average daily alcohol use varies across age and marital status groups. Younger adults, especially those living with a partner or never married, tend to drink more on average, while adults aged 60 and older report the lowest daily alcohol use, regardless of marital status. Overall, alcohol consumption decreases with age, but relationship status—like living with a partner—appears to have a strong influence on drinking habits.

*6. Faceted scatterplot (sleep duration by age, gender, and marital status)*
...
```
This graph shows that married men sleep the most and women with live partners sleep the most.
